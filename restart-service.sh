...
Rebuilds the AfterDraft image, force-recreates the service container, and
...
[[ -r "${ENV_FILE}" ]] || { echo "AfterDraft env file is not readable: ${ENV_FILE}" >&2; exit 1; }

cd "${APP_DIR}"
docker compose --env-file "${ENV_FILE}" config --quiet

if [[ "${CHECK_ONLY}" == 1 ]]; then
  echo "AfterDraft restart configuration is valid."
  exit 0
fi

echo "Building AfterDraft from ${APP_DIR} ..."
docker compose --env-file "${ENV_FILE}" build afterdraft

echo "Recreating the AfterDraft container ..."
...
for attempt in $(seq 1 30); do
  if response="$(curl --fail --silent --show-error "${HEALTH_URL}" 2>/dev/null)" \
    ...
    echo "AfterDraft is healthy. Image: ${image_id:7:12}"
    exit 0
  fi
  sleep 2
done

echo "AfterDraft did not become healthy; recent logs follow:" >&2
