...
async function planAcademicReviewRepairs(input:Parameters<Parameters<typeof registerAcademicRepairPlanner>[0]>[0]){
  const issues=input.issues,targetRefs=[...new Set(issues.flatMap(issue=>issue.targetRefs))];if(!targetRefs.length)throw Object.assign(new Error('Selected review issues have no repairable document targets'),{statusCode:409});if(targetRefs.length>40)throw Object.assign(new Error('Select at most 40 document targets for one repair batch'),{statusCode:400});
  ...
  const html=await readFile(htmlPath,'utf8'),$=cheerio.load(html),fragments=targetRefs.map(targetRef=>{const node=$('[data-block-id]').filter((_index,element)=>$(element).attr('data-block-id')===targetRef).first();if(!node.length)throw Object.assign(new Error(`Repair target no longer exists: ${targetRef}`),{statusCode:409});const serialized=$.html(node);return{targetRef,tag:node.get(0)!.tagName,html:serialized.length>6_000?`${serialized.slice(0,5_980)}…[TRUNCATED]`:serialized}}),evidenceRefs=[...new Set(issues.flatMap(issue=>issue.evidence.map((item:any)=>typeof item?.id==='string'?item.id:'').filter(Boolean)))],images=(await evidenceImagesForIssue(input.jobId,evidenceRefs)).images,reserved=db.prepare("UPDATE import_jobs SET call_count=call_count+1,updated_at=? WHERE id=? AND status='review-ready' AND stage IN ('review','publish-failed') AND call_count+1<=MIN(max_calls,40)").run(now(),input.jobId);
