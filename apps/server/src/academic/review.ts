...
export function planImageBatches(items:EvidenceItem[],maximumImages:number):{batches:EvidenceItem[][];selected:EvidenceItem[];oversize:number}{
  ...
  if(current.length)batches.push(current);return{batches,selected:batches.flat(),oversize};
}
