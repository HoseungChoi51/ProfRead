...
function htmlUpload(filename:string,html:string,documentId?:string,aiReviewEnabled=false):{body:Buffer;contentType:string}{
  const boundary=`afterdraft-${randomUUID()}`,body=Buffer.concat([
    Buffer.from(`--${boundary}\r\nContent-Disposition: form-data; name="sourceKind"\r\n\r\nupload\r\n`),
    Buffer.from(`--${boundary}\r\nContent-Disposition: form-data; name="aiReview"\r\n\r\n${JSON.stringify({enabled:aiReviewEnabled})}\r\n`),
...
describe('academic HTML import lifecycle',()=>{
  it('uploads, previews, and atomically publishes a normalized document',async()=>{
    ...
    const runId=randomUUID(),findingId=randomUUID(),timestamp=now(),blockId='block-shape-test';db.prepare("INSERT INTO model_runs(id,request_id,action,provider_id,model_id,profile,routing_reason,context_tier,status,created_at)VALUES(?,?,?,?,?,?,?,?,?,?)").run(runId,randomUUID(),'import-visual-audit','openai','gpt-5.6-sol','deep','test','import-evidence','completed',timestamp);db.prepare("INSERT INTO import_findings(id,import_job_id,source,issue_code,severity,title,description,target_ref,evidence_json,decision,model_run_id,created_at,updated_at)VALUES(?,?,'model','table-overflow','warning','Table overflow','Needs review',?,'[]','pending',?,?,?)").run(findingId,jobId,blockId,runId,timestamp,timestamp);db.prepare('UPDATE import_jobs SET provenance_json=? WHERE id=?').run(JSON.stringify({qaCoverage:{summary:'All sampled evidence reviewed'}}),jobId);
    const shaped=JSON.parse((await app.inject({method:'GET',url:`/api/import-jobs/${jobId}`,headers:{cookie:headers.cookie}})).body);expect(shaped.coverage).toEqual({summary:'All sampled evidence reviewed'});expect(shaped.findings[0]).toMatchObject({blockId,modelId:'gpt-5.6-sol'});
  ...
  it('applies an accepted corroborated presentation repair without changing scholarly content',async()=>{
    const headers=await authenticatedHeaders(),marker=randomUUID(),upload=htmlUpload('wide-table.html',`<!doctype html><title>Repair ${marker}</title><h1>Repair paper</h1><p>Prose ${marker}</p><table width="1400" style="width:1400px"><tbody><tr><td>Scholarly cell ${marker}</td></tr></tbody></table><math><mi>x</mi><mo>=</mo><mn>1</mn></math>`),created=await app.inject({method:'POST',url:'/api/import-jobs',headers:{...headers,'content-type':upload.contentType},payload:upload.body});
    ...
    const previewPath=join(config.dataDir,'imports',jobId,'preview.html'),preview=cheerio.load(await readFile(previewPath,'utf8')),tableId=preview('table[data-block-id]').first().attr('data-block-id')!,paragraphId=preview('p[data-block-id]').first().attr('data-block-id')!;
    expect(tableId).toBeTruthy();const timestamp=now(),findingId=randomUUID(),unsafeId=randomUUID();
    ...
    const accepted=await app.inject({method:'PATCH',url:`/api/import-jobs/${jobId}/findings/${findingId}`,headers,payload:{decision:'accepted'}});expect(accepted.statusCode,accepted.body).toBe(200);
    expect((await app.inject({method:'PATCH',url:`/api/import-jobs/${jobId}/findings/${findingId}`,headers,payload:{decision:'accepted'}})).statusCode).toBe(200);
    ...
    expect(row<{applied_at:string|null}>('SELECT applied_at FROM import_findings WHERE id=?',findingId)?.applied_at).toBeTruthy();
    expect((await app.inject({method:'PATCH',url:`/api/import-jobs/${jobId}/findings/${findingId}`,headers,payload:{decision:'dismissed'}})).statusCode).toBe(409);
  });
