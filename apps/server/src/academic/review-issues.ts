...
export function listAcademicReviewIssues(jobId:string):AcademicReviewIssue[]{
  const feedback=latestReviewerFeedback(jobId),items=rows<StoredIssue>(`SELECT i.* FROM import_review_issues i WHERE i.import_job_id=? ORDER BY CASE i.verification_status WHEN 'confirmed' THEN 0 WHEN 'unverified' THEN 1 WHEN 'rejected' THEN 2 ELSE 3 END,CASE i.severity WHEN 'error' THEN 0 WHEN 'warning' THEN 1 ELSE 2 END,i.created_at,i.id`,jobId);
...
export async function materializeAcademicReviewIssues(jobId:string,options:{rebuild?:boolean}={}):Promise<AcademicReviewIssue[]>{
  ...
  const time=now();db.exec('BEGIN IMMEDIATE');try{
    ...
    if(options.rebuild){
      const omitted=rows<StoredIssue>('SELECT i.* FROM import_review_issues i WHERE i.import_job_id=? AND NOT EXISTS(SELECT 1 FROM import_review_issue_findings x WHERE x.issue_id=i.id)',jobId),update=db.prepare('UPDATE import_review_issues SET verification_reason=?,proposed_repair_json=?,repairable=?,updated_at=? WHERE id=? AND import_job_id=?');
      for(const issue of omitted){
        if(issue.status!=='pending'||issue.verification_status==='rejected'||issue.verification_status==='resolved')continue;
        const repair=repairPrecondition(parseRepair(issue.proposed_repair_json),map,sourceViews);
        update.run('Carried forward from the previous review because this rebuild did not re-report or verify the issue. Re-adjudicate it or set an explicit reviewer verdict before closing it.',repair?JSON.stringify(repair):null,Number(Boolean(repair)),time,issue.id,jobId);
      }
    }
    db.exec('COMMIT');
