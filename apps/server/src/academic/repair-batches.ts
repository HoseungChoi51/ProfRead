...
function restoreSvgSemantics(candidateHtml:string,sourceHtml:string,targetRef:string):string{
  const targetDoc=loadProfreadHtml(candidateHtml),targetSvg=findSvg(targetDoc,targetRef);if(!targetSvg.length)fail(`SVG repair target no longer exists: ${targetRef}`,409);
  ...
  const expected=sanitizeDocument(sourceHtml,'document.html',()=>null).html,sourceDoc=loadProfreadHtml(expected),sourceSvg=findSvg(sourceDoc,targetRef);if(!sourceSvg.length)fail('The immutable source has no corresponding SVG block',409);const sanitized=sanitizeSvgAsset(sourceDoc.html(sourceSvg)??'');if(!sanitized)fail('The immutable source SVG failed safety validation',409);const safeDoc=loadProfreadHtml(sanitized,{xmlMode:true}),safeSvg=safeDoc('svg').first(),viewBox=validViewBox(safeSvg.attr('viewBox')??safeSvg.attr('viewbox'));if(viewBox)targetSvg.attr('viewBox',viewBox).removeAttr('viewbox');
...
function applyDelegatedProposal(html:string,proposal:AcademicRepairBatchProposal):string{
  ...
  if(proposal.type==='associate-caption'){
    ...
    (caption.get(0)! as any).tagName='figcaption';caption.attr('data-profread-semantic-role','caption');figure.append(caption);return $.html();
  }
  if(proposal.type==='set-semantic-role'){
    const target=findBlock($,proposal.targetRef);if(!target.length||!delegatedTextTags.has(target.get(0)!.tagName))fail('Semantic roles can be assigned only to text blocks');const tag=proposal.role==='title'?'h1':proposal.role==='heading'?'h2':proposal.role==='caption'?'figcaption':'p';(target.get(0)! as any).tagName=tag;target.attr('data-profread-semantic-role',proposal.role);return $.html();
  }
