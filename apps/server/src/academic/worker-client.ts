...
export function prepareReadingPdf(sourcePath:string,range?:{pageStart:number;pageEnd:number},signal?:AbortSignal):Promise<Buffer>{return request('/v1/pdf/prepare',sourcePath,'application/pdf',{filename:basename(sourcePath),...range},signal,18*60_000)}
export function indexReadingPdf(sourcePath:string,options:{sourcePageStart:number;pageStart:number;pageEnd:number;ocr:'auto'|'force'|'off';language:'eng'|'eng+kor'},signal?:AbortSignal):Promise<Buffer>{return request('/v1/pdf/index',sourcePath,'application/pdf',options,signal,10*60_000)}
export function cropReadingPdf(sourcePath:string,options:{page:number;x:number;y:number;width:number;height:number},signal?:AbortSignal):Promise<Buffer>{return request('/v1/pdf/crop',sourcePath,'application/pdf',options,signal)}
