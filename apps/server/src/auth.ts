...
export function registerAuth(app: FastifyInstance): void {
  app.post('/api/auth/login', { config: { rateLimit: { max: 5, timeWindow: '1 minute' } } }, async (request, reply) => {
    ...
    reply.setCookie('co_reader_session', token, { httpOnly: true, secure: config.secureCookies, sameSite: 'strict', path: '/', maxAge: 30 * 24 * 3600 });
    reply.setCookie('co_reader_csrf', csrf, { httpOnly: false, secure: config.secureCookies, sameSite: 'strict', path: '/', maxAge: 30 * 24 * 3600 });
    return { authenticated: true };
  });
  app.post('/api/auth/logout', { preHandler: [authenticate, requireCsrf] }, async (request, reply) => {
    const token = request.cookies.co_reader_session; if (token) db.prepare('DELETE FROM sessions WHERE token_hash=?').run(hash(token));
    reply.clearCookie('co_reader_session', { path: '/' }); reply.clearCookie('co_reader_csrf', { path: '/' }); return { authenticated: false };
  });
  app.get('/api/auth/session', { preHandler: authenticate }, async () => ({ authenticated: true }));
}

export async function authenticate(request: FastifyRequest, reply: FastifyReply): Promise<void> {
  const token = request.cookies.co_reader_session;
...
export async function requireCsrf(request: FastifyRequest, reply: FastifyReply): Promise<void> {
  ...
  const token = request.cookies.co_reader_session; const csrf = request.headers['x-csrf-token'];
