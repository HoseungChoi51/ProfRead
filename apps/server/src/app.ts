...
import { startPdfIndexer, stopPdfIndexer } from './pdf/indexer.js';
...
export async function buildApp() {
  ...
  seedModelSettings();registerDocumentRoutes(app);registerPdfRoutes(app);registerImportJobRoutes(app);registerLibraryRoutes(app);registerThreadRoutes(app);registerWriterRoutes(app);registerRunRoutes(app);registerSettingsRoutes(app);registerKnowledgeRoutes(app);registerExportRoutes(app);registerEditRoutes(app);
  installArxivSourceHook();registerAcademicSourceHook('url',createPublishedSourceHook({convertPdf}));installAcademicReviewHook();
  if(config.backgroundJobsEnabled){startAcademicImportRunner();startPdfIndexer();}app.addHook('onClose',async()=>stopPdfIndexer());
