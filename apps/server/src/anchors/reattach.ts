...
export interface StoredAnchor { exact:string;prefix:string;suffix:string;startOffset:number;blockId?:string }
...
export function reattach(anchor:StoredAnchor,candidates:ReattachCandidate[]):Reattachment{
  ...
  const fuzzy=candidates.map(block=>{const score=similarity(anchor.exact,block.text);return {blockId:block.blockId,startOffset:0,endOffset:block.text.length,score,status:'unmatched' as const}}).sort((a,b)=>b.score-a.score)[0];
