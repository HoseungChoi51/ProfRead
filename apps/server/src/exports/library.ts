...
interface CurrentDocument{id:string;title:string;version:number;source_name:string;created_at:string;edit_revision:number}
...
export async function exportHtmlLibrary(outputDirectory:string,exportedAt=new Date().toISOString()):Promise<{count:number;manifestPath:string}>{
  ...
  for(const document of documents){
    ...
    entries.push({documentId:document.id,title:document.title,version:document.version,editRevision:document.edit_revision,sourceName:document.source_name,sourceImportedAt:document.created_at,filename});
  }
