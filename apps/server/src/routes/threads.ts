...
export function registerThreadRoutes(app: FastifyInstance): void {
  ...
  app.post('/api/threads', async (request,reply)=>{
    const parsed=z.object({documentId:z.string().trim().min(1),anchorId:optionalId,parentMessageId:optionalId,title:z.string().max(240).optional()}).safeParse(request.body);if(!parsed.success)return reply.code(400).send({error:parsed.error.flatten()});
    ...
    if(parsed.data.anchorId){
      const anchor=row<{document_version_id:string;document_id:string}>('SELECT a.document_version_id,v.document_id FROM anchors a JOIN document_versions v ON v.id=a.document_version_id WHERE a.id=?',parsed.data.anchorId);
      ...
      if(anchor.document_version_id!==document.latest_version_id)return reply.code(400).send({error:'Anchor does not belong to the current document version'});
    }
  ...
  app.patch('/api/threads/:id/annotation',async(request,reply)=>{const parsed=z.object({text:z.string().max(500).nullable().optional(),annotationText:z.string().max(500).nullable().optional()}).refine(value=>value.text!==undefined||value.annotationText!==undefined,'Annotation text is required').safeParse(request.body);if(!parsed.success)return reply.code(400).send({error:parsed.error.flatten()});const id=(request.params as{id:string}).id,text=(parsed.data.text??parsed.data.annotationText)?.trim()||null,time=now(),result=db.prepare('UPDATE threads SET annotation_text=?,updated_at=? WHERE id=?').run(text,time,id);return result.changes?{ok:true,text,annotationText:text,updatedAt:time}:reply.code(404).send({error:'Thread not found'});});
