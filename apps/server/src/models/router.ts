...
export interface RouteInput { action: Action; input: string; hasVisual: boolean; webEnabled: boolean; estimatedTokens: number; modelOverride?: string }
...
function capable(model: ModelDefinition, input: RouteInput): boolean {
  ...
  return model.enabled && c.text && c.streaming && (!input.hasVisual || c.vision) && (input.action!=='visualize' || (c.structuredOutput&&c.functionTools)) && (input.action!=='visual-recap'||c.structuredOutput) && (!(input.action==='research'||input.webEnabled)||c.providerWebSearch) && model.contextWindow >= input.estimatedTokens + model.maxOutput;
}
...
export function route(input: RouteInput, models: ModelDefinition[], profiles: ModelProfile[], ttft: Record<string,number> = {}, classifiedProfile?: 'quick'|'standard'|'deep'): RouteDecision {
  if (input.modelOverride) { const model=models.find(m=>m.id===input.modelOverride); if(!model||!capable(model,input))throw new Error('Overridden model cannot satisfy this request');return {profile:deterministicProfile(input)??classifiedProfile??'standard',model,reason:'manual override',eligible:[model]}; }
