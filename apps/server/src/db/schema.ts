...
export function createImportJobsTableSql(name='import_jobs',ifNotExists=true):string{
 ...
 article_title TEXT,
 article_fallback_url TEXT,
 article_ai_boundary INTEGER NOT NULL DEFAULT 1,
 article_selection_json TEXT,
 selected_page_start INTEGER,
 selected_page_end INTEGER,
 boundary_model_run_id TEXT REFERENCES model_runs(id) ON DELETE SET NULL,
 fallback_used INTEGER NOT NULL DEFAULT 0,
 fallback_reason TEXT,
 status TEXT NOT NULL CHECK(status IN ('queued','converting','awaiting-selection','review-ready','finalizing','published','failed','cancelled')),
 stage TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS import_findings (
 ...
 decision TEXT NOT NULL DEFAULT 'pending' CHECK(decision IN ('pending','accepted','dismissed','manual')),
 applied_at TEXT,
