...
import { BRIDGE,READER_CSS,normalizeAssetPath,sanitizeDocument,sanitizeStylesheet,textFragmentsForOffsets } from './sanitize.js';
...
it('ships a syntactically valid sandbox bridge with direct editing and image resize messages',()=>{expect(()=>new Function(BRIDGE)).not.toThrow();expect(BRIDGE).toContain('edit-context');expect(BRIDGE).toContain('finish-editing');expect(BRIDGE).toContain("type:'resize-image'");expect(BRIDGE).toContain('startEditing(text.dataset.blockId)')});
...
it('renders and refreshes accessible semantic anchor marks',()=>{
  ...
  expect(BRIDGE).toContain('exactRange.toString()===a.exact');
...
it('reconciles visible thread previews without changing anchor text offsets',()=>{
  ...
  expect(BRIDGE).toContain("existing.map(part=>part.textContent||'').join('')!==a.exact");
...
it('wraps multi-node selections as one logical accessible anchor',()=>{
  ...
  expect(BRIDGE).toContain("const fallbackStart=(el.textContent||'').indexOf(a.exact)");
});
