...
describe('sanitization',()=>{
  ...
  it('preserves native MathML, semantic source identity, and safe scholarly links',()=>{const result=sanitizeDocument(`<p>Inline <math alttext="x squared"><semantics><msup><mi>x</mi><mn>2</mn></msup><annotation encoding="application/x-tex">secret-source</annotation></semantics></math>.</p><math display="block" alttext="integral"><mrow><mo>∫</mo><mi>f</mi></mrow><annotation encoding="application/x-tex">\\int f</annotation></math><p data-profread-source-ref="section:intro">Text <a href="#references" target="_blank">[1]</a> <a href="https://doi.org/10.1/example">DOI</a> <a href="javascript:alert(1)">bad</a></p><h2 id="references">References</h2>`,'paper.html',()=>null);expect(result.html).toContain('<math');expect(result.html).toContain('<msup>');expect(result.html).toContain('alttext="integral"');expect(result.html).toContain('href="#references"');expect(result.html).not.toMatch(/href="#references"[^>]*target=/);expect(result.html).toContain('href="https://doi.org/10.1/example"');expect(result.html).toContain('<a href="#" data-original-href="javascript:alert(1)"');expect(result.canonicalText).toContain('Inline x2.');expect(result.canonicalText).not.toContain('secret-source');expect(result.canonicalText).toContain('integral');const again=sanitizeDocument(`<p data-profread-source-ref="section:intro">Changed presentation</p>`,'paper.html',()=>null);expect(again.blocks[0]?.id).toBe(result.blocks.find(block=>block.text.includes('Text'))?.id)});
  ...
  it('wraps wide tables in a keyboard-scrollable browsing container',()=>{const result=sanitizeDocument('<table><caption>Results</caption><tr><th>A</th><td>1</td></tr></table>','paper.html',()=>null);expect(result.html).toContain('class="profread-table-scroll"');expect(result.html).toContain('aria-label="Scrollable table"');expect(result.html).toContain('.profread-table-scroll>table');expect(result.blocks).toEqual(expect.arrayContaining([expect.objectContaining({type:'table',text:'ResultsA1'})]))});
...
it('renders and refreshes accessible semantic anchor marks',()=>{
  expect(READER_CSS).toContain('.profread-highlight-important');
  expect(READER_CSS).toContain('.profread-highlight-question');
  expect(READER_CSS).toContain('text-decoration-style:wavy');
  expect(READER_CSS).toContain('.profread-highlight-comment');
  expect(READER_CSS).toContain('text-decoration-style:dotted');
  expect(READER_CSS).toContain('.profread-definition');
...
it('reconciles visible thread previews without changing anchor text offsets',()=>{
  ...
  expect(READER_CSS).toContain('.profread-discussion[data-preview]::after');
  expect(READER_CSS).toContain('.profread-definition[data-preview]::after');
  expect(READER_CSS).toContain('.profread-discussion:focus-visible');
