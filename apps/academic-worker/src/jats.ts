...
export async function convertJats(body:Buffer,options:JatsConversionOptions):Promise<OperationResult>{
  ...
  const root=await mkdtemp(join(tmpdir(),'profread-jats-')),bundle=join(root,'bundle'),input=join(root,'article.xml');await mkdir(bundle);await writeFile(input,body,{mode:0o600});
  try{
    ...
    manifest.files=await bundleFiles(bundle);await writeJson(join(bundle,'manifest.json'),manifest);const archive=join(root,'result.zip');await createZip(bundle,archive,options.signal);await assertFileWithinWorkerOutputLimit(archive,'Converted JATS bundle exceeds the worker response limit.');return{root,archivePath:archive,downloadName:'profread-jats-bundle.zip'};
